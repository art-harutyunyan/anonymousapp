  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      ...
      const { data: matchRows } = await db
        .from("matches")
        .select(
          "id, user_a, user_b, created_at, last_read_at_a, last_read_at_b",
        )
      ...
      const enriched: MatchRow[] = await Promise.all(
        matchRows.map(
          async (m: {
            ...
            const lastReadAt =
              (m.user_a === user.id ? m.last_read_at_a : m.last_read_at_b) ??
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="sticky top-0 z-10 border-b border-border bg-background/80 backdrop-blur-xl px-4 h-14 flex items-center gap-3">
          ...
          {matches.length > 0 && (
            <Badge
              variant="outline"
              className="border-primary/25 text-primary text-xs ml-auto bg-primary/8"
            >
              {matches.length}
            </Badge>
          )}
          ...
          {loading ? (
            <div className="flex flex-col gap-3 max-w-xl">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 p-4 bg-white/80 border border-black/[0.06] rounded-2xl"
                >
            ...
            <div className="flex flex-col items-center justify-center h-full py-20 text-center">
              ...
              <p className="text-foreground/45 text-sm max-w-xs leading-relaxed">
                Head to Discover and press &ldquo;Start Talking&rdquo; on
                someone. When they do the same, you&apos;ll match!
              </p>
            </div>
          ) : (
